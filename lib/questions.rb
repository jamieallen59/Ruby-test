...
def select_elements_starting_with_vowel(array)
	array.select { |word| word[0].match(/[aeiou]/) }
end
...
def array_sort_by_last_letter_of_word(array)
			reversed = array.map { |x| x.reverse }
			sorted = reversed.sort
			righted = sorted.map { |x| x.reverse }
end
...
def separate_array_into_even_and_odd_numbers(array)
	positive = array.select { |x| x % 2 == 0}
	negative = array.select { |x| x % 2 == 1}
	[positive, negative]
end
...
def convert_array_to_a_hash(array)
	second_array = array.each_slice(2).to_a
	Hash[second_array]
end
...
def add_together_keys_and_values(hash)
	array = [hash.keys, hash.values].flatten
	array.inject { |x, y| x + y }
end
...
def titleize_a_string(string)
	...
	array.map { |word| word == 'a' || word == 'and' || word == 'the' ? word : word.capitalize }.join(' ')
end
